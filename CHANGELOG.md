...
- **Dashboard UI**: Token saver configuration panel with real-time budget tracking
...
- **Dynamic Python Discovery**: Windows Python 3.10-3.14+ detection across all install locations:
  - `%LOCALAPPDATA%\Python\pythoncore-*-64\Scripts`
  - `%LOCALAPPDATA%\Programs\Python\Python*\Scripts`
  - `%APPDATA%\Python\*\Scripts`
  - Traditional `C:\Python*` paths
- **IPv4/IPv6 Fallback**: Reliable health probes handle localhost resolution differences
- **Process Lifecycle**: Start, stop, status, and log management APIs
...
- **Success Rate Tracking**: Rolling success rate per model for reliability scoring
- **Verified Badges**: Dashboard UI shows latency, tested status, and re-test triggers
- **Accept/Reject Feedback**: Track suggestion quality for continuous improvement
...
- **Commit-Lint Skill**: Conventional commits enforcement with auto-formatting
- **Dynamic Skills Registry**: Load and validate custom skills at runtime
- **Extended Hub UI**: Enhanced skill management with backup/restore functionality
- **Custom Studio**: Interactive skill editor with live preview
